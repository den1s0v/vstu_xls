...
class PatternComponent(WithCache, WithSafeCreate):
    ...
    # Вес компонента (-∞, ∞) для регулирования вклада в точность опредления родителя. >0: наличие желательно, 0: безразлично (компонент может быть опущен без потери точности), <0: наличие нежелательно (в этом случае должен быть опциональным!).
    weight: float = 1
