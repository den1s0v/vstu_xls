...
class PatternComponent:
    ...
    constraints: list[SpatialConstraint]  # ограничения, накладываемые на расположение этого компонента по отношению к родителю. Используются координаты текущего дочернего элемента и родительского элемента. Также могут использоваться координаты других компонентов родителя, которые были определены по списку компонентов выше этого компонента.
    ...
    _constraints_with_full_names: list[SpatialConstraint] = None
    
    def get_constraints_as_for_parent(self) -> list[SpatialConstraint]:
        if not self._constraints_with_full_names:
            self._constraints_with_full_names = [
        ...
        return self._constraints_with_full_names
