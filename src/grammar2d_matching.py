...
class NonTerminalMatcher(ElementMatcher):
    """
        ???
    """
    element: NonTerminal
    ...
    def match(self, precision_ratio_cutoff=0.9) -> list[Match2d]:
        ...
        m = Match2d(self.element, component2match={}, data=adict(constraint=True))
        # self._match_tree = [[m]]
        match_tree = [[m]]
        ...
        for component in self.element.components:
            ...
            if not new_level and not component.optional:
                raise ValueError(
                    'CANNOT match component {component.name} for element {self.element.name}. Consider weaken restrictions on it or making it optional.')
    ...
    def _incremental_match_component(self, partial_match: Match2d, component: PatternComponent,
        ...
        candidate_matches = self.grammar_matcher.matches_by_element[component.element]
        new_matches = []
