...
class Point(namedtuple('Point', ['x', 'y'])):
    ...
    def manhattan_distance_to(self, other: 'Point', per_axis=False) -> int:
        ...
        try:
            other.manhattan_distance_to(self, per_axis=per_axis)
        except AttributeError as exc:
            raise ValueError(f'Cannot calculate distance from `{type(self).__name__}` to `{type(other).__name__}`.', other, exc)
