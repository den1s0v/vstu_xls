"""Набросок структуры программы и API для автоматического повторения ручных корректировок данных, преимущественно строковых.
    ...
    Передача гипотез:
    - Вызывающий код может передать опциональный параметр `hypotheses: list[Item]` в функцию `apply_correction`
    - Все переданные гипотезы сохраняются в БД как CorrectObject через `find_or_create_correct_object`
    - Гипотезы проверяются на соответствие наравне с найденными в БД CorrectObject
    - Score вычисляется стандартно, без приоритета
    - Сохраненные гипотезы становятся доступны для последующих вызовов `apply_correction`
    ...
    Создание CorrectObject:
    CorrectObject могут создаваться тремя способами:
    1. Из гипотез, переданных в `apply_correction` (через `find_or_create_correct_object`) - все гипотезы сохраняются в БД
    2. Вручную администратором через интерфейс
    3. Автоматически из полной копии Occurrence (если не найдено подходящих CorrectObject)
...
def apply_correction(
    occurrence: Occurrence, 
    scope_id: int = 0,
    hypotheses: Optional[list[Item]] = None
    ...
    # 0.5. Обработка гипотез (если переданы):
    	- Для каждой гипотезы (Item) вызвать `find_or_create_correct_object` для преобразования в CorrectObject
    	- Все гипотезы сохраняются в БД как CorrectObject (даже если не используются для Resolution)
    	- Добавить полученные CorrectObject к списку кандидатов для проверки соответствия
    ...
    # 2. Если не найдено - выполнить анализ и:
    	...
    	- при поиске учитывать также гипотезы (уже преобразованные и сохраненные в БД),
    ...
    Score для Resolution с гипотезами вычисляется стандартно, без бонусов.
    ...
    # 5. Вернуть CorrectObject или None, если не удалось ничего создать и связать в статусе не-INVALID.
    """
    pass
