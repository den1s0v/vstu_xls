"""Набросок структуры программы и API для автоматического повторения ручных корректировок данных, преимущественно строковых.
    ...
    Список (таблица) Корректировок — основная web-страница для человека (администратора, ревьювера).
     ...
     — Фильтрация по статусу (ожидает, подтверждена, аннулирована — множественный выбор): чтобы можно было вывести в начало таблицы строки с любым статусом, а остальные скрыть.
     
     
    Форма редактирования Корректировки.

    Может быть выполнена в виде модального окна или отдельной страницы.
    Необходимые изменения в структуре содержимого формы (добавление и удаление гипотез) должны выполняться по нажатию кнопок здесь же, без обращения к серверу.
