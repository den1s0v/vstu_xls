"""Набросок структуры программы и API для автоматического повторения ручных корректировок данных, преимущественно строковых.
    ...
    1. АЛГОРИТМЫ И ВЫЧИСЛЕНИЯ:
       - Формула вычисления score для Resolution: "10*(оценка нечёткой близости по value) + взвешенная сумма совпавших элементов контекста"
         * Как именно вычисляется "нечёткая близость по value"? (Levenshtein, Jaro-Winkler, другие метрики?)
         * Какие веса для элементов контекста? (использовать score из ContextElement?)
         * Нормализация значений score?
       ...
       - Логика "покрытия контекста" для Occurrence:
         * "набор элементов контекста которого полностью покрывает элементы контекста кандидата" -
           это означает, что все элементы кандидата должны присутствовать в существующем с теми же значениями?
         * Или достаточно совпадения по важным элементам (important=True)?
         * Как обрабатывать отсутствующие элементы (с учетом absence_allowed)?

       - Обновление кеша resolved_to:
         * Когда именно пересчитывается? (при каждом изменении Resolution, CorrectObject, или по расписанию?)
         * Что означает "актуальный по дате"? (какой период актуальности?)
         * Нужна ли инвалидация кеша при изменении связанных объектов?
